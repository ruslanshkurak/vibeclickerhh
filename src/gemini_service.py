...
class GeminiService:
    ...
    def evaluate_vacancy(self, resume: str, vacancy_text: str) -> dict:
        ...
        special_wishes = Config.USER_SPECIAL_WISHES.strip() if hasattr(Config, 'USER_SPECIAL_WISHES') and Config.USER_SPECIAL_WISHES else ""
        wishes_prompt_block = f"""
        ОСОБЫЕ ПОЖЕЛАНИЯ И СТРОГИЕ КРИТЕРИИ КАНДИДАТА:
        \"\"\"
        {special_wishes}
        \"\"\"
        """ if special_wishes else ""

        # Быстрая проверка на стоп-условие "работа только/чисто в офисе" (если соискатель не хочет в офис)
        vac_text_lower = vacancy_text.lower()
        if special_wishes:
            wishes_lower = special_wishes.lower()
            if any(term in wishes_lower for term in ["не откликаться на работа чисто в офисе", "без офиса", "только удаленка", "не в офисе", "без офиса"]):
                office_triggers = ["чисто в офисе", "только в офисе", "только офис", "работа в офисе (без удаленки)", "удаленка не предусмотрена", "гибрид и удаленка не рассматриваются"]
                if any(trig in vac_text_lower for trig in office_triggers):
                    return {"score": 0, "reason": "Дисквалифицировано: вакансия с форматом 'чисто в офисе', что противоречит критериям кандидата."}

        prompt = f"""
        Ты — опытный ИТ-рекрутер. Твоя задача — сопоставить резюме соискателя с вакансией и оценить, насколько кандидат подходит на эту роль по шкале от 0 до 10.

        ОБРАТИ ВНИМАНИЕ НА ГРЕЙД И ОСОБЫЕ ПОЖЕЛАНИЯ / ПРИОРИТЕТЫ КАНДИДАТА:
        ...
        ПРАВИЛА ОЦЕНКИ, ПООЩРЕНИЯ И ДИСКВАЛИФИКАЦИИ (ШКАЛА 0-10):
        1. [ДИСКВАЛИФИКАЦИЯ -> 0 из 10]: Если в особых пожеланиях соискателя указаны явные стоп-условия или запреты (например, "не откликаться на работа чисто в офисе", "без офиса"), И вакансия нарушает их (например, формат "чисто в офисе"), СРАЗУ ставь оценку 0 из 10.
        2. [БОНУС / НАЧИСЛЕНИЕ БАЛЛОВ (+1..+3)]: Если в особых пожеланиях кандидата указаны плюсы, приоритеты или предпочтительные технологии/условия (например, "удаленка", "микросервисы", "Docker", "гибкий график", "FastAPI"), И вакансия содержит эти требования/условия — НАКИДЫВАЙ дополнительные баллы к базовой оценке сопоставления стека (вплоть до 10 из 10).
        3. [ГРЕЙД -> 0 из 10]: Если в тексте вакансии явно указано, что ищется уровень из неподходящих ({', '.join(Config.EXCLUDE_GRADES)}), сразу ставь оценку 0 из 10.
        ...
        - "score": целое число от 0 до 10 (где 10 — идеально сопоставленный стек с учетом всех желаемых бонусов кандидата, а 0 или 1 — вакансия с нежелательными стоп-условиями или неподходящим грейдом).
        - "reason": краткое пояснение (1-2 предложения), почему выставлена такая оценка (отметь начисленные бонусы за пожелания или причины снижения/дисквалификации).
        """
